...
const MultiMetricCard = ({data,wDivider}) => {
    ...
    return (
        <div className="flex flex-row">
            {data ? data.map( (element,index) => 
                {
                    return (
                        <div key={index} className="flex flex-row w-1/3 items-center mr-3 pr-3">
                            ...
                            { wDivider ? ((index === data.length-1) ? '' : <div className="divider"/>) : ''}    
                        </div>
                    )
                }
            ) : ''}
